...
module.exports = async function (ctx) {
  ...
  while (START < Number.MAX_VALUE) {
    ...
    await curly.get(`https://estudy-filkom.upiyptk.ac.id//ujian/masuk/${START}/${user.commands.kode}/${user.commands.krs}`, {
      httpHeader: [
      ...
      if (Array.from(document.querySelectorAll('h4')).find(el => el.textContent === 'Error 404 - Page Not Found')) {
        await replaceMessage(ctx, user.message, `Searching Ke : ${START}`)
        START++
      } else {
        await ctx.reply(`https://estudy-filkom.upiyptk.ac.id//ujian/masuk/${START}/${user.commands.kode}/${user.commands.krs}`)
